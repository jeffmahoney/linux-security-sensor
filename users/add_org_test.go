...
func (self *UserManagerTestSuite) TestAddUserToOrg() {
	...
	err := users.AddUserToOrg(
		...
		"UserO1", "UserO1", map[string]*acl_proto.ApiClientACL{"O2": admin_policy})
	assert.ErrorContains(self.T(), err, "PermissionDenied")

	// Can an admin in O1 just add a user to O2?
	err = users.AddUserToOrg(
		self.Ctx, users.UseExistingUser,
		"AdminO1", "UserO1", map[string]*acl_proto.ApiClientACL{"O2": admin_policy})
	assert.ErrorContains(self.T(), err, "PermissionDenied")

	// Can an OrgAdmin add a user from O1 to O2?
	err = users.AddUserToOrg(
		self.Ctx, users.UseExistingUser,
		"OrgAdmin", "AdminO1", map[string]*acl_proto.ApiClientACL{"O2": admin_policy})
	...
	err = users.AddUserToOrg(
		self.Ctx, users.UseExistingUser,
		"AdminO1", "UserO1", map[string]*acl_proto.ApiClientACL{"O2": reader_policy})
	assert.NoError(self.T(), err)
	...
	err = users.AddUserToOrg(
		self.Ctx, users.UseExistingUser,
		"OrgAdmin", "NoSuchUser", map[string]*acl_proto.ApiClientACL{"O2": admin_policy})
	assert.ErrorContains(self.T(), err, "User not found")

	// Request a new user record to be created.
	err = users.AddUserToOrg(
		self.Ctx, users.AddNewUser,
		"AdminO2", "NoSuchUser", map[string]*acl_proto.ApiClientACL{"O2": reader_policy})
	assert.NoError(self.T(), err)
	...
	err = users.AddUserToOrg(
		self.Ctx, users.AddNewUser,
		"AdminO2", "VelociraptorServer", map[string]*acl_proto.ApiClientACL{"O2": reader_policy})
