...
// the ListenerBytes object using ListenerBytes.Send(). If a receiver is ready
...
// ListenerBytes is able to go back into direct delivering mode.
type ListenerBytes struct {
	mu sync.Mutex
	...
	// ListenerBytes context.
	ctx    context.Context
	cancel func()
}

// Should not block - very fast.
func (self *ListenerBytes) Send(item []byte) {
	...
	if self.file_buffer_active {
		self.file_buffer.EnqueueSerialized(item)
		return
	}
...
func (self *ListenerBytes) _file_buffer_ready() chan bool {
	self.mu.Lock()
	defer self.mu.Unlock()
...
func (self *ListenerBytes) _switchToFileMode() {
	if !self.file_buffer_active {
...
func (self *ListenerBytes) _switchToDirectMode() {
	if self.file_buffer_active {
...
func (self *ListenerBytes) Output() chan []byte {
	return self.output
}
...
func (self *ListenerBytes) Close() {
	...
	if self.file_buffer != nil {
		...
		items := self.file_buffer.LeaseSerialized(self.options.FileBufferLeaseSize)
...
func (self *ListenerBytes) Debug() *ordereddict.Dict {
	self.mu.Lock()
	defer self.mu.Unlock()
...
func NewListenerBytes(
	config_obj *config_proto.Config,
	ctx context.Context, name string,
	options api.QueueOptions) (*ListenerBytes, error) {
	...
	go func() {
		for {
			...
			case <-self._file_buffer_ready():
				...
				self.mu.Lock()
				items := self.file_buffer.LeaseSerialized(lease_size)
				if len(items) == 0 {
