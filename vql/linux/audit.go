...
func (self *AuditWatcherService) setupWatcherService() error {
	...
	err = self.openListener()
	if err != nil {
		self.commandClient.Close()
		return err
	}

	self.listenSocketBufSize, err = unix.GetsockoptInt(self.listenFd, unix.SOL_SOCKET,
							   unix.SO_RCVBUF)
	if err != nil {
...
func (self *AuditWatcherService) openListener() error {
	...
	err = syscall.Bind(fd, src)
	if err != nil {
		syscall.Close(fd)
		return fmt.Errorf("Could not bind to netlink socket: %w", err)
	}
...
func (self *AuditWatcherService) closeListener() {
	if self.listenFd >= 0 {
		syscall.Close(self.listenFd)
		self.listenFd = -1
	}
}
...
func (self *AuditWatcherService) resetListenSocketBufSize(bufSize int) error {
	...
	if bufSize == 0 {
		...
		if bufSize == 0 {
			bufSize, err = unix.GetsockoptInt(self.listenFd, unix.SOL_SOCKET,
							  unix.SO_RCVBUF)
			if err != nil {
				self.logger.Warn("audit: could not get socket receive buffer size: %v", err)
				return err
	...
	err = unix.SetsockoptInt(self.listenFd, unix.SOL_SOCKET, unix.SO_RCVBUFFORCE, bufSize)
	if err != nil {
...
func (self *AuditWatcherService) startListener() error {
	...
	err = unix.EpollCtl(self.epollFd, unix.EPOLL_CTL_ADD, self.listenFd,
...
func (self *AuditWatcherService) ReceiveMessageBuf(msgBuf *AuditMessageBuf) error {
	...
	nr, from, err := unix.Recvfrom(self.listenFd, msgBuf.Data, flags)
	if err != nil {
